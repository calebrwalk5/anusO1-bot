...
const PREFIX = "!anus ";
const COMMANDS = ["`!anus in front of commands`", "`help`", "`ping`","`coinflip`", "`lol`", "`messages`", "`random`", "`meme`"];
...
client.on('message', message => {
  ...
  if (!message.sender === JELLY) {
	...
	console.log("random");
	  ...
	  console.log("poggers");
  } else if (message.content.startsWith(`${PREFIX}nukeplsanus`)) {
	  var nuke = 1;
	  while (nuke = 1) {
